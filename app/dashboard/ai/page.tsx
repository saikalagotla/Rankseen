...
const ENGINE_META: Record<
  string,
  { label: string; icon: string; minPlan: "free" | "starter" | "pro" }
> = {
...
function buildGeoTips(params: {
  ...
  const unlockedMissing = engines.filter(
    (e) => !e.isLocked && e.hasResults && e.mentionCount === 0,
  );
  const googleMissing = unlockedMissing.find((e) => e.key === "google_ai");
  const llmMissing = unlockedMissing.filter((e) =>
    ["claude", "chatgpt"].includes(e.key),
  );
  ...
  if (aiCompetitors.length > 0) {
    ...
    for (const c of aiCompetitors)
      freq.set(c.competitor_name, (freq.get(c.competitor_name) ?? 0) + 1);
...
export default async function AIVisibilityPage() {
  ...
  const engineOrder = ["google_ai", "claude", "chatgpt", "perplexity", "bing"];
  ...
  const totalMentions = unlockedWithData.reduce(
    (a, e) => a + e.mentionCount,
    0,
  );
  const totalQueries = unlockedWithData.reduce(
    (a, e) => a + e.queries.length,
    0,
  );
  const mentionRate =
    totalQueries > 0 ? Math.round((totalMentions / totalQueries) * 100) : null;
    ...
    <div className="p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="animate-fade-in flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            When customers ask AI assistants for {bizType}s in {city}, is{" "}
            {bizName} mentioned?
        ...
        {!isDemo && (
          <div className="flex items-center gap-3">
            {userPlan !== "pro" && (
              <UpgradeButton
                label={
                  userPlanRank === 0 ? "Upgrade to Starter" : "Upgrade to Pro"
                }
                variant="button"
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="animate-fade-in-up bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
          ...
          <p className="text-3xl font-bold text-slate-900 dark:text-white mb-1">
            {unlockedWithData.length}{" "}
            <span className="text-lg text-slate-400 dark:text-slate-500">
              / 5
            </span>
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {5 - unlockedWithData.length}{" "}
            {userPlan === "pro" ? "not yet scanned" : "locked by plan"}
          </p>
        </div>
        <div className="animate-fade-in-up delay-75 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
          ...
          <p
            className={`text-3xl font-bold mb-1 ${hasData ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
          >
            {mentionRate !== null ? `${mentionRate}%` : "—"}
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {hasData
              ? `${totalMentions} of ${totalQueries} test queries`
              : "No scan data yet"}
          </p>
        </div>
        <div className="animate-fade-in-up delay-150 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
          ...
          <p
            className={`text-3xl font-bold mb-1 ${bestPosition ? "text-blue-600 dark:text-blue-400" : "text-slate-400 dark:text-slate-500"}`}
          >
      ...
      <div className="mb-6">
        {(() => {
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                    Businesses AI engines recommend instead of or alongside you
                  </p>
                </div>
                <div className="px-6 py-10 text-center">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500">
                    Run an AI scan to see who&apos;s being recommended.
                  </p>
                </div>
              </div>
            );
          ...
          const freq = new Map<
            string,
            { count: number; engines: Set<string> }
          >();
          for (const c of aiCompetitors) {
                  ...
                  <div className="space-y-2.5">
                    {leaderboard.map(([name, { count, engines }], i) => (
                      <div key={name} className="flex items-center gap-3">
                        <span
                          className={`w-6 h-6 rounded-md text-xs font-bold flex items-center justify-center shrink-0 ring-1 ${
                            i === 0
                        ...
                        <span className="text-sm text-slate-700 dark:text-slate-300 flex-1 font-medium truncate">
                          {name}
                        </span>
                ...
                <div className="divide-y divide-slate-50 dark:divide-slate-800">
                  {Array.from(byEngineComp.entries()).map(([engine, comps]) => {
                    ...
                    for (const c of comps) {
                      if (
                        !seen.has(c.competitor_name) ||
                        seen.get(c.competitor_name)! > c.position
                      ) {
                      ...
                      <div key={engine} className="px-6 py-4">
                        ...
                        <div className="flex flex-col gap-2">
                          {unique.map(([name, position]) => (
                            <div key={name} className="flex items-center gap-3">
                              <span
                                className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold ring-1 shrink-0 ${
                                  position === 1
                                    ? "bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 ring-amber-200 dark:ring-amber-800"
                                    : position <= 3
                                      ? "bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 ring-emerald-200 dark:ring-emerald-800"
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
            ...
            >
              <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {!engine.isLocked && engine.hasResults && (
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                        Mentioned in {engine.mentionCount} of{" "}
                        {engine.queries.length} queries
                      </p>
                    )}
                  </div>
                </div>
                {engine.isLocked ? (
                  ...
                  <span
                    className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg ring-1 ${
              ...
              {engine.isLocked ? (
                <div className="px-6 py-6 flex flex-col items-center text-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                      {engine.label} is a Pro feature
                    </p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                      Upgrade to see how you rank when customers ask{" "}
                      {engine.label} for local {bizType}s.
                    </p>
                  </div>
                ...
                <div className="px-6 py-6 flex flex-col items-center text-center gap-2">
                  <p className="text-sm text-slate-400 dark:text-slate-500">
                    No scan data yet for {engine.label}.
                  </p>
                  {!hasAnyKey && (
                    <p className="text-xs text-slate-400 dark:text-slate-500">
                      Configure the API key above to enable this engine.
                    </p>
                  )}
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="shrink-0 flex flex-col items-end gap-1">
                        ...
                        {q.mentioned && q.excerpt && (
                          <p className="text-xs text-slate-400 dark:text-slate-500 max-w-xs text-right line-clamp-1">
                            {q.excerpt}
                          </p>
                        )}
                      </div>
        ...
        <div className="animate-fade-in-up delay-300 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden self-start">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
              Tailored for {bizName} in {city}
            </p>
          </div>
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {geoTips.map((tip, i) => (
              <div key={i} className="px-5 py-5 flex gap-3">
                <span className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <div>
