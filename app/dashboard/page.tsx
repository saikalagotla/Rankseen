...
export default async function DashboardPage() {
  ...
  // --- Citation summary (exclude manual-only platforms) ---
  const MANUAL_CITATION_PLATFORMS = new Set(['Apple Maps', 'Bing Places'])
  const autoCitations = citations.filter(c => !MANUAL_CITATION_PLATFORMS.has(c.platform))
  const citationIssues = autoCitations.filter(c => c.status !== 'ok').length
  const citationOk = autoCitations.filter(c => c.status === 'ok').length
  const citationHealth = autoCitations.length
    ? Math.round((citationOk / autoCitations.length) * 100)
    : null
  ...
  } else if (autoCitations.length === 0) {
    digestItems.push({
      color: 'gray',
  ...
  if (aiResults.length > 0) {
    ...
    if (unmentionedEngine && userPlanRank >= 1) {
      const engineLabel = { claude: 'Claude', chatgpt: 'ChatGPT', bing: 'Bing Copilot' }[unmentionedEngine] ?? unmentionedEngine
      digestItems.push({
        color: 'amber',
        text: (
          <>
            <span className="font-medium">Your business was not mentioned</span> when customers asked{' '}
            {engineLabel} for local {bizType.toLowerCase()}s.{' '}
            Check your AI Visibility tab for actionable tips.
          </>
        ),
        link: { label: 'View AI report', href: '/dashboard/ai' },
      })
    } else if (userPlanRank === 0) {
      digestItems.push({
        color: 'gray',
        text: (
          <>
            <span className="font-medium">AI Visibility:</span>{' '}
            Upgrade to Starter or Pro to track whether Claude, ChatGPT, and Bing recommend you when customers search for local {bizType.toLowerCase()}s.
          </>
        ),
        link: { label: 'Upgrade to Starter', href: '/setup', checkout: true },
      })
    }
  } else {
    digestItems.push({
      ...
      text: (
        <>
          ...
          {userPlan === 'pro'
            ...
            : userPlan === 'starter'
            ? 'Run an AI scan to see if Perplexity, Google AI, and Claude recommend your business.'
            : 'Upgrade to Starter to start tracking AI visibility for your business.'}
        </>
      ),
      link: userPlanRank >= 1
        ? { label: 'Run AI scan', href: '/dashboard/ai' }
        : { label: 'Upgrade to Starter', href: '/setup', checkout: true },
    })
  }
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Keyword</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Rank</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Chg</th>
                </tr>
              </thead>
              <tbody>
                {keywordSummaries.slice(0, 5).map((row, i) => (
                  <tr key={row.keyword} className={`${i < Math.min(keywordSummaries.length, 5) - 1 ? 'border-b border-slate-50 dark:border-slate-800' : ''} hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors`}>
                    <td className="px-5 py-3 text-sm text-slate-700 dark:text-slate-300 truncate max-w-[140px]">{row.keyword}</td>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          {autoCitations.length === 0 ? (
            ...
            <div className="divide-y divide-slate-50 dark:divide-slate-800">
              {autoCitations.slice(0, 6).map((c) => (
                <div key={c.platform} className="px-5 py-3.5 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    {c.status === 'ok' ? (
                      ...
                      <svg className="w-4 h-4 text-slate-300 dark:text-slate-600 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 rounded-2xl p-6 flex flex-col">
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-amber-100 dark:border-amber-900/30">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{weekLabel}</p>
            </div>
            <span className="ml-auto shrink-0 bg-white dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 text-xs font-semibold px-2 py-0.5 rounded-full">
              {digest.length} items
            </span>
          </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="grid grid-cols-3 lg:grid-cols-5 divide-x divide-y divide-slate-100 dark:divide-slate-800">
            {(['perplexity', 'google_ai', 'claude', 'chatgpt', 'bing'] as const).map((engine) => {
              ...
              const minRank = { perplexity: 0, google_ai: 0, claude: 1, chatgpt: 2, bing: 2 }[engine] ?? 0
              const isLocked = minRank > userPlanRank
              ...
              return (
                <div key={engine} className={`p-4 flex flex-col gap-2 ${isLocked ? 'bg-slate-50 dark:bg-slate-800/50' : ''}`}>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3">
            {reviews.map((review, idx) => (
              <div key={review.id} className={`px-6 py-5 ${idx < reviews.length - 1 ? 'border-b sm:border-b-0 sm:border-r border-slate-100 dark:border-slate-800' : ''}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-semibold text-slate-600 dark:text-slate-400 shrink-0">
                        {(review.author ?? '?').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{review.author ?? 'Anonymous'}</p>
                        <div className="flex items-center gap-1.5">
                          <p className="text-xs text-slate-400 dark:text-slate-500 capitalize">{review.source}</p>
                          {review.rating && (
                            <div className="flex">
